...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    const userLimit = await env.AI_USER_RATE_LIMITER.limit({ key: `recommend-actions:${actor}` });
    if (!userLimit.success) return json({ error: "RATE_LIMITED" }, 429, { "retry-after": "60" });
    const globalLimit = await env.AI_GLOBAL_RATE_LIMITER.limit({ key: "recommend-actions" });
    ...
    console.error("Workers AI action recommendation failed", error instanceof Error ? error.message : "Unknown error");

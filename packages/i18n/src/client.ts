...
const jaMessagesWithoutDailyLife = {
  ...
  otherAnswers: {
    area: { label: "滞在している区市町村を入力", placeholder: "例：世田谷区（正確な住所・施設名は不要）", required: "「その他」を選んだ場合は入力が必要です。", notice: "区市町村までにしてください。正確な住所や施設名は入力しません。この内容はWorkers AIへ送りません。" },
    nationality: { label: "国籍または地域を入力", placeholder: "例：タイ", required: "「その他」を選んだ場合は入力が必要です。", notice: "答えたくない場合は別の選択肢を使えます。この内容はWorkers AIへ送りません。" },
    visitPurpose: { label: "その他の来日目的を入力", placeholder: "例：イベントへの参加、家族の手伝い", required: "「その他」を選んだ場合は入力が必要です。", notice: "次のカード候補を選ぶため、この内容だけをCloudflare Workers AIへ送ります。氏名、連絡先、書類番号、正確な住所は入力しません。" },
    family: { label: "一緒にいるその他の家族を入力", placeholder: "例：親、きょうだい", required: "「その他家族がいる」を選んだ場合は入力が必要です。", notice: "家族の氏名は入力しません。この内容はWorkers AIへ送りません。" },
  },
  actions: {

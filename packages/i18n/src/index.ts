...
export type LocaleReviewStatus = "draft" | "reviewed";
...
export type UserMessages = {
  ...
  ui: {
    ...
    localeOptions: { ja: string; en: string; my: string };
...
const jaMessages = {
  metadata: { label: "Japanese", nativeLabel: "日本語", status: "reviewed", updatedAt: "2026-08-22", reviewedAt: "2026-08-22", reviewedBy: "StayBridge team" },
  ui: {
    skip: "本文へ移動", navSteps: "わたしのステップ", navLocal: "近くの支援", navHelp: "相談先", crisis: "行政・支援者向け Preparedness View",
    ...
    back: "戻る", next: "次へ", finish: "状況を整理する", selectMany: "あてはまるものをすべて選べます", optional: "分かる範囲で大丈夫です",
    reviewed: "今の状況を整理しました", reviewedIntro: "まず、滞在についての公式な確認と、当面の生活に必要な情報を順番に見ていきます。",
    seeRoadmap: "次のステップを見る", answerAgain: "回答を見直す", roadmapTitle: "あなたの次のステップ", roadmapIntro: "重要な順に並べました。すべてを今日終える必要はありません。",
    why: "なぜこの案内？", source: "根拠となる情報", verified: "確認日", human: "個別の確認が必要", localTitle: "この地域で確認できる場所",
    ...
    details: "公式サイトを見る", sourceLabel: "Open Data source", updated: "データ更新", unavailable: "公開日不明", backToRoadmap: "ステップへ戻る", continueToHelp: "相談先へ進む",
    schoolNote: "入学・就学については自治体または教育機関への確認が必要です。", noResources: "この地域は現在、StayBridgeのMVPでは詳細な地域データに対応していません。",
    helpTitle: "人に相談する", helpIntro: "この内容は、あなたの滞在状況によって手続が変わる可能性があります。StayBridgeでは最終的な判断はしません。公式の相談窓口へ確認してください。",
    ...
    homeLabel: "StayBridge Tokyo ホーム", primaryNavLabel: "主要ナビゲーション", languageSelectTitle: "MVP静的翻訳プレビュー", languageSelectLabel: "言語・静的翻訳プレビュー",
    ...
    supportFallback: "対応内容・言語・受付時間は公式ページで確認してください。", resourceIcons: { school: "学", medical: "+", child_support: "こ", public_facility: "公" }, localeOptions: { ja: "日本語", en: "English", my: "မြန်မာ" },
...
export const userMessages = { ja: jaMessages, en: enMessages, my: myMessages } satisfies UserMessagesByLocale;
...
export function assertValidUserMessages(value: unknown): asserts value is UserMessagesByLocale {
  ...
  for (const locale of supportedUserLocales) {
    ...
    if (!["draft", "reviewed"].includes(catalog.metadata.status)) throw new Error(`Invalid review status at ${locale}.metadata.status`);
    assertNonEmpty(catalog.metadata.updatedAt, `${locale}.metadata.updatedAt`);
    if (catalog.metadata.status === "reviewed") {
      assertNonEmpty(catalog.metadata.reviewedAt, `${locale}.metadata.reviewedAt`);
      assertNonEmpty(catalog.metadata.reviewedBy, `${locale}.metadata.reviewedBy`);
    }

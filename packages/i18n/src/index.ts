...
export const selectableUserLocales = reviewedUserLocales;
...
export function assertValidUserMessages(value: unknown): asserts value is UserMessagesByLocale {
  ...
  for (const locale of supportedUserLocales) {
    ...
    for (const reviewKey of ["internalReview", "expertReview"] as const) {
      ...
      const expectedReviewStatus = reviewKey === "internalReview" && shouldBeReviewed ? "reviewed" : "pending";
      if (review.status !== expectedReviewStatus) throw new Error(`Invalid ${reviewKey} review status at ${locale}.metadata`);
    }

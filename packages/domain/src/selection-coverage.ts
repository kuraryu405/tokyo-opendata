...
import { isoCountryCodes, tokyoWardCodes } from "./assessment-options";

export const assessmentOptionCodes = {
  municipality: tokyoWardCodes,
  nationality: [...isoCountryCodes, "OTHER"],
  visitPurpose: ["tourism", "visiting_family_or_friends", "work", "study", "resident", "other"],
  departureWindow: ["within_7_days", "within_30_days", "within_3_months", "after_3_months", "no_departure_plan"],
  returnStatus: ["possible", "difficult", "unknown"],
  stayAnswer: ["known", "unknown"],
  family: ["none", "children", "spouse", "other"],
  childAge: ["0-2", "3-5", "6-11", "12-14", "15-17", "18+"],
  accommodation: ["hotel", "family_or_friend", "rental", "temporary_facility", "other"],
  needs: ["stay", "consultation", "accommodation", "living_cost", "employment", "education", "childcare", "medical", "daily_life", "language", "other", "none"],
...
export const selectionCoverage: readonly SelectionCoverage[] = [
  ...
  ...(["work", "study", "resident", "other"] as const).map((code) => entry("visitPurpose", code, "rule_input", "Uses the non-short-visit difficult-return branch; the answer is not treated as an official status.", ["R-STAY-RETURN-DIFFICULT-OTHER", "R-CONSULT-RETURN-DIFFICULT-OTHER"])),
  ...
  ...(["within_3_months", "after_3_months", "no_departure_plan"] as const).map((code) => entry("departureWindow", code, "rule_input", "Uses the later short-visit branch; otherwise causes no card by itself.", ["R-STAY-RETURN-DIFFICULT-SHORT-LATER"])),
  ...
  ...(["family_or_friend", "rental", "temporary_facility", "other"] as const).map((code) => entry("accommodation", code, "explicit_no_card", "No housing risk is inferred from this answer alone; an accommodation concern can still match.")),
  ...
  entry("needs", "other", "summary_only", "Preserves an uncategorized concern in the consultation summary without inferring eligibility or urgency."),

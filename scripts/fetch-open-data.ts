/**
 * Refresh the reproducible, bundled Open Data caches used by the demo.
 * Run with: pnpm data:fetch
 *
 * The app never fetches these URLs at runtime. This script deliberately keeps
 * selection separate from adaptation: the cache stores only fields published
 * by the verified source rows and fails if a selected source row disappears.
 */
import { mkdir, writeFile } from "node:fs/promises";
import { join } from "node:path";
import { inflateRawSync } from "node:zlib";
import { adaptResourceRecord, adaptTokyoForeignPopulation, type RawResourceRecord } from "../src/data/adapters/open-data";
import type { LocalResource, LocalResourceCategory, LocalResourcesCache, PopulationCache } from "../src/data/adapters/types";
...
function selectResources(records: RawResourceRecord[], selected: SelectedResource[]): LocalResource[] {
  return selected.map((selection) => {
    ...
    const resource = adaptResourceRecord(record, {
      id: selection.id,
...
async function main() {
  ...
  const populationCache: PopulationCache = {
    ...
    records: [{
      ...myanmar,
      raw: {
        "地域階層": String(rawPopulation["地域階層"] ?? ""),
        "地域コード": String(rawPopulation["地域コード"] ?? ""),
        "国・地域(人)": String(rawPopulation["国・地域(人)"] ?? ""),
        "ミャンマー": String(rawPopulation["ミャンマー"] ?? ""),
      },
    }],
    coverageNotes: [
  ...
  const schoolRecords = csvRecords(new TextDecoder("shift_jis").decode(schoolBytes));
  const standardFiles = extractZipFiles(standardArchive);
  const resources = [
    ...selectResources(schoolRecords, schoolSelection),
  ...
  await Promise.all([
    writeFile(join(outputDirectory, "kita-myanmar-population.json"), `${JSON.stringify(populationCache, null, 2)}\n`),

...
export function generateActions(situation: Situation, context: RuleContext = {}): Action[] {
  ...
  const add = (
    ...
    actions.set(id, { ...actionCatalog[id], priority, reasonCode, reasonText, ...(localResourceCategories ? { localResourceCategories } : {}), ...(timing ? { timing } : {}) });
  };

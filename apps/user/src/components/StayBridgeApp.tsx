...
export function StayBridgeApp({ route: initialRoute = defaultRoute, assessmentDate }: { route?: StayBridgeRoute; assessmentDate: string }) {
  ...
  }, [aiRecommendation, answeredSteps, familyAnswers, isDemoSituation, otherAnswers, situation, stayAnswer, storageReady]);

  const actions = useMemo(() => {
    if (!assessmentComplete) return [];
    const ruleActions = generateActions(situation, { asOfDate: assessmentDate, stayAnswer });
  ...
  }, [aiRecommendation, assessmentComplete, assessmentDate, otherAnswers.visitPurpose, situation, stayAnswer]);
...
function ImmediateStatus({ locale, t, situation, stayAnswer, familyAnswers, otherAnswers, answeredSteps, persistence, isDemo, persist, declinePersistence, deletePersistence, roadmap, edit }: { locale: Locale; t: UserCopy; situation: Situation; stayAnswer: StayAnswer; familyAnswers: FamilyAnswers; otherAnswers: OtherAnswers; answeredSteps: number[]; persistence: SituationPersistenceState; isDemo: boolean; persist: () => void; declinePersistence: () => void; deletePersistence: (credentials: SavedRecordCredentials) => void; roadmap: () => void; edit: () => void }) {

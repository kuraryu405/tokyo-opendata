...
Human Support画面の公式窓口と相談準備カードを主導線とし、その下で任意にAIチャットを開ける。AIは窓口で説明する内容と確認する質問の整理に限定する。Situation Checkの回答は自動送信せず、チャット入力と直近8件までの会話だけをCloudflare Workers AIへ送る。会話は再読み込みで消去し、ユーザーは画面上からも消去できる。モデル未接続、推論失敗、レート超過時は公式相談先を使うよう案内し、他の画面・機能を停止しない。
